...
export default class Cognito {
  ...
  public async signUpUser(email: string, password: string, userAttr: Array<any>): Promise<ResponseData> {
    var params = {
    ...
    try {
      ...
      const response: ResponseData = { status: true, message: "Success", statusCode: 200, data: data };
      return response;
    } catch (error) {
      const response: ResponseData = { status: false, message: "Please try again", statusCode: 500, error };
      return response;
    }
  }
  public async signInUser(email: string, password: string): Promise<ResponseData> {
    var params = {
  ...
  public async confirmSignUp(email: string, code: string): Promise<ResponseData> {
    var params = {
